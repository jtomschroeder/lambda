...
PROBLEM(E2) {
   ...
   const auto solution = sum(fibonacci | view::filter(even) |
                             view::take_while(curry(flip(std::less<>{}), 4e6)));

   SOLUTION(solution, 4613732);
}

uint64_t smallest_factor(uint64_t n) {
   auto factors = view::closed_ints(2ull, n) >>=
       [n](auto i) { return yield_if(n % i == 0, i); };
...
PROBLEM(E3) {
   SOLUTION(largest_prime_factor(600851475143), 6857);
}

// Find the largest palindrome made from the product of two 3-digit numbers.
PROBLEM(E4) {
   // list comprehension of product of two 3-digit numbers
   auto products = view::ints(100, 1000) >>= [](int x) {
      return view::ints(x, 1000) >>= [=](int y) { return yield(x * y); };
   };
...
PROBLEM(E5) {
   SOLUTION(ranges::accumulate(view::closed_ints(1, 20), 1, lcm), 232792560);
}
...
PROBLEM(E7) {
   const auto solution =
       (view::ints(1) | view::filter(prime) | view::drop(10000)).front();
